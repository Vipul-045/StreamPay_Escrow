...
impl SubscriptionEscrow {
    ...
    /// Gelato resolver function - called every block off-chain
    /// Returns: (canExec, execPayload) 
    pub fn checker(&self, subscriber: Address) -> (bool, Bytes) {
        // Check if any subscription is due for this subscriber
        ...
        while subscription_id < max_subscriptions {
            // Check if this subscription belongs to the subscriber and is active
            if self.subscription_subscriber.get(subscription_id) == subscriber 
                ...
                // Check if payment is due and user has sufficient balance
                if current_time >= last_payment + plan_interval && user_balance >= plan_price {
                    // Encode call to process_payment(subscription_id)
                    let mut exec_payload = Vec::new();
                    // Function selector for processSubscriptionPayment(uint256)
                    exec_payload.extend_from_slice(&[0x8d, 0x96, 0x7d, 0x8b]);
                    
                    // Encode subscription_id as uint256 (32 bytes)
    ...
    /// Called by Gelato bots when payment is due
    pub fn process_payment(&mut self, subscription_id: U256) -> Result<bool, SubscriptionError> {
        // This is just an alias to our existing function for Gelato compatibility
        self.process_subscription_payment(subscription_id)
    }
    
    pub fn process_subscription_payment(&mut self, subscription_id: U256) -> Result<bool, SubscriptionError> {
        let caller = self.vm().msg_sender();
        
        // Only admin can process payments (simplified access control)
